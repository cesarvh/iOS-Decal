...
class PlayerViewController: UIViewController {
    ...
    */
    func playOrPauseTrack(sender: UIButton) {
        ...
        if (sender.selected == true) {
            ...
            let currentSong = player.currentItem
            if (currentSong == nil) { // if loading for first time
                let song = AVPlayerItem(URL: url)
                player.replaceCurrentItemWithPlayerItem(song)
                player.play()
            } else { // if not loading for the first time
                if currentSong!.status == .ReadyToPlay {
                    player.play()
                } else {
    ...
    */
    func nextTrackTapped(sender: UIButton) {
        // first increment the index of the track we want
        currentIndex! += 1
        // check if there is a next track. we do this by checking if num of track we want == num of total tracks
        ...
        player = AVPlayer(playerItem: newSong)
    ...
    */
    ...
    func previousTrackTapped(sender: UIButton) {
        // if the song is 3 seconds in then go back to the beggining
        if (player.currentTime().seconds > 3) {
            player.seekToTime(CMTimeMakeWithSeconds(0, 1))
            
        }
            // otherwise go back to previous song
        else {
